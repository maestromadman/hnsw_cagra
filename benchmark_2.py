...
DATA_DIR = "data"   # set by prepare_data.py; falls back to random if absent


def _load_data(n_vectors: int, n_queries: int, dim: int):
    """
    Load real embeddings from DATA_DIR if available, otherwise generate random.

    Files expected: data/embeddings_{dim}.npy  (N, dim) float32
                    data/queries_{dim}.npy      (M, dim) float32
    Both generated by prepare_data.py.  Corpus is subsampled to n_vectors rows;
    queries subsampled to n_queries rows so every experiment parameter combination
    is supported without re-running the encoder.
    """
    c_path = os.path.join(DATA_DIR, f"embeddings_{dim}.npy")
    q_path = os.path.join(DATA_DIR, f"queries_{dim}.npy")

    if os.path.exists(c_path) and os.path.exists(q_path):
        corpus  = np.load(c_path, mmap_mode="r")
        qfile   = np.load(q_path, mmap_mode="r")
        vectors = _f32(corpus[:n_vectors])
        queries = _f32(qfile[:n_queries])
        print(f"    [data] loaded real embeddings  dim={dim}  "
              f"n={n_vectors:,}  nq={n_queries}", flush=True)
    else:
        rng     = np.random.default_rng(SEED)
        vectors = _f32(rng.standard_normal((n_vectors, dim)))
        queries = _f32(rng.standard_normal((n_queries, dim)))
        print(f"    [data] using random Gaussian  dim={dim}  "
              f"n={n_vectors:,}  nq={n_queries}", flush=True)
